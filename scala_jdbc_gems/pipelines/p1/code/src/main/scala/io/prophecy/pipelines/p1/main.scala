...
object Main {
  ...
  def apply(context: Context): Unit = {
    val df_jdbc_databricks            = jdbc_databricks(context)
    val df_jdbc_username              = jdbc_username(context)
    val df_jdbc_environment_variables = jdbc_environment_variables(context)
  }
