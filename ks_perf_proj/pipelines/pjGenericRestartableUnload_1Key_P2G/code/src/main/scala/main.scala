...
object Main {
  ...
  def apply(context: Context): Unit = {
    val df_db2GenericTableUnload = db2GenericTableUnload(context)
    val df_moFieldNamesLkp       = moFieldNamesLkp(context, df_db2GenericTableUnload)
  ...
  def main(args: Array[String]): Unit = {
    ...
    spark.conf.set("prophecy.metadata.pipeline.uri",
                   "pipelines/pjGenericRestartableUnload_1Key_P2G"
    )
    registerUDFs(spark)
