...
import graph.pscKeyGenerationVC_PIB_ID.config._
import graph.pscKeyGenerationVC_PIB_ID.pscMessageHandlingSCtrC235
...
import java.time._
package object pscKeyGenerationVC_PIB_ID {

  def apply(context: Context, lkPostDominoMIAs: DataFrame): DataFrame = {
    ...
    val df_trTrimNK_stage_var         = trTrimNK_stage_var(context, lkPostDominoMIAs)
