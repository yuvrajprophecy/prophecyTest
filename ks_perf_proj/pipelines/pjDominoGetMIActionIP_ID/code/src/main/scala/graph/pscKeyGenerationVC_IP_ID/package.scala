...
import graph.pscKeyGenerationVC_IP_ID.config._
import graph.pscKeyGenerationVC_IP_ID.pscMessageHandlingSCtrC235
...
import java.time._
package object pscKeyGenerationVC_IP_ID {

  def apply(context: Context, lkGetIP: DataFrame): DataFrame = {
    ...
    val df_trTrimNK_stage_var         = trTrimNK_stage_var(context, lkGetIP)
