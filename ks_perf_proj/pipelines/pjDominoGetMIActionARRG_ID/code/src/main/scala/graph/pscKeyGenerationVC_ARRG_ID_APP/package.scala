...
import graph.pscKeyGenerationVC_ARRG_ID_APP.config._
import graph.pscKeyGenerationVC_ARRG_ID_APP.pscMessageHandlingSCtrC235
...
import java.time._
package object pscKeyGenerationVC_ARRG_ID_APP {

  def apply(context: Context, lkGenArrgIdApp: DataFrame): DataFrame = {
    ...
    val df_trTrimNK_stage_var         = trTrimNK_stage_var(context, lkGenArrgIdApp)
