...
import graph.pscKeyGenerationVC_ARRG_ID.config._
import graph.pscKeyGenerationVC_ARRG_ID.pscMessageHandlingSCtrC235
...
import java.time._
package object pscKeyGenerationVC_ARRG_ID {

  def apply(context: Context, lkGetARRG_ID: DataFrame): DataFrame = {
    ...
    val df_trTrimNK_stage_var         = trTrimNK_stage_var(context, lkGetARRG_ID)
