...
def aka_alxaa2_Query_wit(spark: SparkSession) -> DataFrame:
    return spark.read\
        .format("jdbc")\
        .option("url", f"{Config.jdbcUrl_aka_alxaa2_Query_wit}")\
        .option("user", f"{Config.username_aka_alxaa2_Query_wit}")\
        .option("password", f"{Config.password_aka_alxaa2_Query_wit}")\
        .option(
          "query",
